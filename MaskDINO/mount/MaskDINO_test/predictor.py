...
class VisualizationDemo(object):
    ...
    def extract_regions(self, predictions, min_area: int = 10):
        # create pseudo-instances
        if "sem_seg" in predictions:
            sem_logits = predictions["sem_seg"]              # (C, H, W)
            sem_labels = sem_logits.argmax(dim=0)            
            sem_probs = sem_logits.softmax(dim=0)            
            ...
            for class_id in np.unique(sem_labels2):
                ...
                for idx in range(1, num_labels):
                    ...
                    # avg confidence
                    scores.append(float(sem_scores2[instance_mask].mean()))
