...
from roman.map.maskdino_wrapper_count import FastSAMWrapper
...
class ROMANMapRunner:
    ...
    def update(self, t: float, run_name: str): 
        t0 = self.img_data.t0
        ...
        img_time, observations, pose_odom_camera, img, frame_descriptor = self.update_fastsam(t, run_name)
        update_t1 = time.time()
    ...
    def update_fastsam(self, t, run_name):
        ...
        # run MaskDINO
        observations, frame_descriptor = self.fastsam.run(img_t, run_name, T_odom_camera, img, depth_data=depth_data)
