...
class FastSAMWrapper:
    ...
    def run(self, t, run_name, pose, img, depth_data=None):
        """
        ...
        # run MaskDINO
        masks = self._process_img(img, ignore_mask=ignore_mask, t=t, run_name=run_name)
    ...
    def _process_img(self, image_bgr, ignore_mask=None, keep_mask=None, t=None, run_name=None):
        img = image_bgr
        ...
        sem_logits = outputs["sem_seg"]
        sem_labels = sem_logits.argmax(dim=0)
        sem_labels2 = sem_labels.cpu().numpy().astype("int32")
        masks = []

    
        T = 0.3
        sem_probs = (sem_logits / T).softmax(dim=0)
        sem_scores, _ = sem_probs.max(dim=0)
        sem_scores2 = sem_scores.cpu().numpy().astype("float32")

        if run_name is None or str(run_name).strip() == "":
            run_name = "run"
        csv_filename = f"{run_name}_instance_class_scores.csv"
        write_header = (not os.path.exists(csv_filename)) or (os.path.getsize(csv_filename) == 0)

        # open 1x per frame (veel sneller)
        with open(csv_filename, "a", newline="") as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(["run_name", "t", "instance_class_id", "inst_score"])
            ...
            for class_id in np.unique(sem_labels2):
                ...
                for idx in range(1, num_labels):
                    ...
                    if self.prob_threshold is not None and inst_scores < float(self.prob_threshold):
                        continue

                    masks.append(instance_mask.astype("uint8"))

                    t_val = "" if t is None else f"{float(t):.8f}"
                    w.writerow([run_name, t_val, int(class_id), inst_scores])

        if not masks:
            return []

        segmask = np.stack(masks, axis=0)
